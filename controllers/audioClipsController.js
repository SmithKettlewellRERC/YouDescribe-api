...
const convertISO8601ToSeconds =
  require("../shared/helperFunctions").convertISO8601ToSeconds;
...
const audioClipController = {
          ...
          (errCopy) => {
            ...
            if (audioDescriptionId.length === 24) {
              ...
              AudioDescription.findOneAndUpdate(
                ...
                (errUpdateAd, returnedAudioDescription) => {
                  if (errUpdateAd) {
                      ...
                      (errToUpdateAd, audioClipUpdated) => {
                        ...
                        Video.findOneAndUpdate(
                          ...
                          .exec((errPopulate, video) => {
                            console.log(
                              "ALL SET 1 - Create audio clip - AD Already exists - Video already exists"
                            );
                ...
                Video.findOne({ youtube_id }, (errFindVideo, video) => {
                  ...
                  if (video) {
                    ...
                    Video.update(
                      ...
                      {
                        $set: { updated_at: nowUtc() },
                        $push: { audio_descriptions: createdAdId },
                      },
                            ...
                            (errUpdatingAudioClip, audioClipUpdated) => {
                              ...
                              if (audioClipUpdated) {
                                ...
                                Video.findOne({ youtube_id })
                                  .populate({
                                    path: "audio_descriptions",
                                    populate: { path: "user audio_clips" },
                                  })
                                  .exec((errPopulate, video) => {
                                    console.log(
                                      "ALL SET 2 - Create audio clip - Create AD - Video already exists"
                                    );
                                    const ret =
                                      apiMessages.getResponseByCode(1005);
                                    ret.result = video;
                                    res.status(ret.status).json(ret);
                                  });
                              } else {
                                ...
                                (errUpdatedAD, updatedAD) => {
                                  if (updatedAD) {
                                    ...
                                    Video.findOne({ _id: newVideoIdCreated })
                                      ...
                                      .exec((errPopulate, video) => {
                                        console.log(
                                          "ALL SET 3 - Create audio clip - Create AD - Create Video"
                                        );
                                        const ret =
                                          apiMessages.getResponseByCode(1005);
                                        ret.result = video;
                                    ...
                                    const ret =
                                      apiMessages.getResponseByCode(1);
                                    res.status(ret.status).json(ret);
                                  }
                                }
                              );
  ...
  delOne: (req, res) => {
    ...
    AudioClip.findByIdAndRemove({ _id: audioClipId }).exec(function (
      errFindAndRemove,
      audioClipDeleted
    ) {
      if (errFindAndRemove) {
  ...
  updateOne: (req, res) => {
    ...
    AudioClip.findOneAndUpdate({ _id: acId, user: userId }, toUpdate, {
      new: true,
    }).exec((err, ac) => {
      if (err) {
        console.log(err);
  ...
  startSpeechToText: async (req, res) => {
    const audiofile = req.file;

    const filename = audiofile.path;
    const encoding = "WAV";
    const sampleRateHertz = 1411;
  ...
  getSpeechToTextResult: (req, res) => {
    ...
    request.get(
      `https://www.listenbycode.com/api/v1/gettranscript?appkey=${appkey}&resulttype=sentence&orderid=${orderId}`,
        ...
        transcriptions.forEach((item) => {
          ...
          AudioClip.findOne({ _id: audioClipId }, (err, audioClip) => {
            if (audioClip) {
              ...
              var path =
                directory + audioClip.file_path + "/" + audioClip.file_name;
              if (fs.existsSync(path)) {
                ...
                Transcription.findOneAndUpdate(
                  ...
                  console.log(
                    "not found:" + path + "; audio_clip:" + audioClipId
                  );
                });
              }
            }
  ...
  getTranscriptionResult: (req, res) => {
    ...
    var failed = 0;
    var pending = 0;
    Transcription.find({ status: "pending" })
          ...
          request.get(
            ...
            function optionalCallback(err, response, body) {
              ...
              if (jsonObj.status == "success") {
                ...
                console.log(
                  "failed:" + ++failed + "; " + JSON.stringify(jsonObj)
                );
              } else {
                console.log(
                  "pending:" + ++pending + "; " + JSON.stringify(jsonObj)
                );
              }
            }
          );
  ...
  addOneByAI: (req, res) => {
    ...
    const fileName = `${youtubeId}_${audioClipId}.${
    ...
    fse.move(req.file.path, finalFilePath, { overwrite: true }, (err) => {
      ...
      mm.parseFile(finalFilePath, { duration: true }).then((metadata) => {
        ...
        const duration = parseFloat(
          parseFloat(metadata.format.duration).toFixed(2) || 0.0
        );
        ...
        User.findOne({ email: email }).exec((err, user) => {
          ...
          Video.findOne({ youtube_id: youtubeId }).exec((err, video) => {
            ...
            AudioDescription.findOne({ user: userId, video: videoId }).exec(
              (err, audioDescription) => {
                if (audioDescription) {
                  ...
                  function optionalCallback(err, response, body) {
                    if (!err) {
                      ...
                      if (jsonObj.items.length > 0) {
                        ...
                        const description =
                          jsonObj.items[0].snippet.description;
